...
<div class="space-y-6">
    ...
    @if($groupedByCategory)
        ...
        @foreach($groupedByCategory as $categoryGroup)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <!-- Group by confidence -->
                @php
                    $matched = array_filter($categoryGroup['attributes'], fn($a) => $a['confidence'] === 'HIGH' && $a['matched_attribute_id']);
                ...
                @if(!empty($matched))
                    <div class="mb-6">
                        <h4 class="text-sm font-semibold mb-2 text-green-600 dark:text-green-400">✓ Eşleşti (HIGH)</h4>
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach($matched as $item)
                                        <tr>
                                            ...
                                            <td class="px-4 py-2 text-sm text-green-600 dark:text-green-400 font-medium">{{ $item['matched_attribute_code'] }}</td>
    ...
    @elseif($analysis)
        ...
        @if(!empty($matched))
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold mb-4 text-green-600 dark:text-green-400">✓ Eşleşti (HIGH Confidence)</h3>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($matched as $item)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-green-600 dark:text-green-400 font-medium">{{ $item['matched_attribute_code'] }}</td>
